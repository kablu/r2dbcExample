...
public class R2dbcPostgresqlConfiguration extends AbstractR2dbcConfiguration {
  ...
  public PostgresqlConnectionFactory connectionFactory() {
	...
	/*
	 * @Bean public CommandLineRunner commandLineRunner(UserRepository
	 * userRepository) { return args -> { DatabaseClient databaseClient =
	 * DatabaseClient.create(connectionFactory());
	 * asList("DROP TABLE IF EXISTS users;",
	 * "CREATE TABLE users (id serial primary key, name varchar);") .forEach(s ->
	 * databaseClient.execute() .sql(s) .fetch() .rowsUpdated() .block() );
